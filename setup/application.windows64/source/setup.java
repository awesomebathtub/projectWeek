...
public void setup(){
  ...
  for (int i = 0; i < healthboosts.length; i++){
    int rand1 = PApplet.parseInt(random(-5000,5000));
    int rand2 = PApplet.parseInt(random(-5000,5000));
...
class Player {
  ...
  public void ammoDisplay (){
    ...
    text(eqWep + "s left:", xPos+150, yPos+250);
    
  }
  
  public void monsterKillsDisplay (){
    fill(0);
    text("Monster Kills:   " + monsterKills, xPos-200,yPos+275);
  }
}//end of player class
...
class WeaponBox{
  ...
  public void pickUp(){ 
    if(player.xPos+16 >= xPos && player.xPos+16 <= xPos + 32 && player.yPos+16 >= yPos && player.yPos+16 <= yPos+32 && d == false){
      d = true;
      pickedUp = true;
      int which = PApplet.parseInt(random(1370));
      if(which <= 500){
        eqWep = "Tomahawk";
